...
tokens = reserved + (
    'SKIP', 'VARIABLE', 'NUMBER', 'COMMENT', 'STRING', 'ISTRING', 'ZSTRING', 'scatterer', 
)
...
def snapToQuadrant(angle):
    ...
    if np.fabs(angle) < eps: angle = 0
    if np.fabs(angle-np.pi/2) < eps: angle = np.pi/2
    if np.fabs(angle-np.pi) < eps: angle = np.pi
    if np.fabs(angle-3./2.*np.pi) < eps: angle = 3./2.*np.pi
    if np.fabs(angle-2*np.pi) < eps: angle = 2*np.pi
...
def processGDMLTorus(args) :
    ...
    if solid2Name == None:  # no a2 to 2pi, so 0 to a2 is simply all of torus
...
def processFile(fileName):
    ...
    patscatter = re.compile('^scatter\w*\(')
